...
class PasswordResetConfirmFlow:
    ...
    async def execute(self, command: PasswordResetConfirmCommand) -> PasswordResetConfirmResult:
        ...
        try:
            ...
            result = await self._confirm_password_reset_step(command, context)
            context.mark_password_changed(result.identity_id, result.email)
            ...
            if self._should_send_confirmation_email():
                ...
                await self._send_confirmation_email_step(result, context)
    ...
    async def _confirm_password_reset_step(self, command: PasswordResetConfirmCommand, context: PasswordResetConfirmContext):
        ...
        """
        from core.identity.dto.contracts import PasswordResetConfirmCommand as IdentityResetConfirmCommand
        
        reset_cmd = IdentityResetConfirmCommand(
            token=command.token,
            new_password=command.new_password,
            new_password_confirm=command.new_password_confirm,
        )
        
        result = await self.identity_service.reset_password_with_token(reset_cmd)
        return result
    
    async def _send_confirmation_email_step(self, reset_result, context: PasswordResetConfirmContext):
        """
        ...
        cmd = SendNotificationCommand(
            template_key="password_reset_confirmation",
            channel=Channel.EMAIL,
            recipient=reset_result.email,
            language="en",
            sender_key=self.config["templates"]["password_reset_confirmation"].get("sender_key"),
            context={"email": reset_result.email},
        )
        
        log = await self.notification_service.send_from_dto(cmd)
